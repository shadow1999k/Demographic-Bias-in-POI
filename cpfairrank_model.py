...
export LD_LIBRARY_PATH=/home/srahmani/anaconda3/envs/py3/lib
...
parser.add_argument('-ug','--usergroup', help='user grouping method', required=True)
...
def load_data(train_data, test_data):
  ...
  eval_method = BaseMethod.from_splits(
      train_data=train_data, test_data=test_data, rating_threshold=0, exclude_unknowns=True, verbose=True
  )
...
def read_ground_truth(test_file):
  ...
  for eachline in truth_data:
    ...
    if iid in eval_method.train_set.iid_map.keys():
      # convert uids to uidx
      uid = eval_method.train_set.uid_map[uid]
...
def load_ranking_matrices(model, total_users, total_items, topk):
  ...
  for uid in tqdm(range(total_users)):
    # updated with the sorted values
    S[uid] = sorted(model.score(uid), reverse=True)
...
def fairness_optimisation(fairness='N', uepsilon=0.000005, iepsilon = 0.0000005):
  ...
  for i in V1:
    # idcg per user: based on the length of actual set for each user
    user_idcg_i = 1
    if i in ground_truth.keys():
      for idx in range(2, len(ground_truth[i])):
        user_idcg_i += 1 / np.log2(idx + 1)
...
for dataset in ds_names:
  ...
  train_data, test_data = read_data(dataset=dataset)
  ...
  for u_group in ds_users:
    ...
    active_user_ids = read_user_groups(user_group_fpath = f"user_groups/{dataset}/{u_group}/{dataset}_g1.txt", gid = 0)
    inactive_user_ids = read_user_groups(user_group_fpath = f"user_groups/{dataset}/{u_group}/{dataset}_g2.txt", gid = 1)
    ...
    for i_group in ds_items:
      ...
      for model in exp.models:
        results = open(f"results/results_{dataset}_{u_group}_{model.name}.csv", 'w')
        ...
        for fair_mode in ['N', 'C']:
          if fair_mode == 'N':
            ...
            for user_eps in [0.0001, 0.0005, 0.001, 0.005, 0.001, 0.005, 0.01, 0.05]:
